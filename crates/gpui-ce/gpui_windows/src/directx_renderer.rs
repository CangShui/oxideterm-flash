...
pub(crate) mod shader_resources {
    ...
    pub(super) fn build_shader_blob(entry: ShaderModule, target: ShaderTarget) -> Result<ID3DBlob> {
        ...
        const SHADERS_MAIN: &str = include_str!("shaders.hlsl");
        const COLOR_TEXT_RASTER: &str = include_str!("color_text_raster.hlsl");
        ...
        unsafe {
            ...
            // Both shader files include alpha_correction.hlsl; inline it so the
            // in-memory compile has no external include to resolve.
            let source =
                shader_source.replacen("#include \"alpha_correction.hlsl\"", ALPHA_CORRECTION, 1);
            ...
            let source_name = b"shaders.hlsl\0";
