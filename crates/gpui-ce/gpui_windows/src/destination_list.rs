...
/// ICU's `u_strlen` equivalent for the null-terminated UTF-16 buffer
/// written by `IShellLinkW::GetDescription`. Avoiding the ICU import keeps
/// the binary free of the non-system `icuuc.dll` dependency.
unsafe fn utf16_buffer_len(ptr: *const u16) -> usize {
    let mut len = 0;
...
fn create_destination_list() -> anyhow::Result<(ICustomDestinationList, Vec<SmallVec<[PathBuf; 2]>>)>
{
    ...
    for i in 0..count {
        ...
        let description = {
            ...
            let len = unsafe { utf16_buffer_len(buffer.as_ptr()) };
