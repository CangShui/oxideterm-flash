...
pub fn load_settings_from_path(path: &Path) -> Result<SettingsLoadResult> {
    ...
    // A durable rewrite per load costs an fsync+rename on every process start.
    // A missing file must be materialized and a corrupt file must stay intact;
    // an existing envelope is rewritten only when sanitization actually
    // changed its payload. Synthesized fallback payloads never compare equal
    // reliably, so they bypass the dirty check entirely.
    enum PersistOnLoad {
        Always,
        IfChanged,
        Never,
    }
    let (raw, updated_at, persist) = match read_envelope(path) {
        Ok(Some((raw, updated_at))) => (raw, updated_at, PersistOnLoad::IfChanged),
        Ok(None) => (
            PersistedSettings::default().to_value(),
            now_ms(),
            PersistOnLoad::Always,
        ),
        Err(err) => {
    ...
    let sanitized = match persist {
        PersistOnLoad::Never => sanitize_settings_value(raw)?,
        PersistOnLoad::Always => {
            let sanitized = sanitize_settings_value(raw)?;
            write_envelope(path, &sanitized.settings, updated_at)?;
            sanitized
        }
        PersistOnLoad::IfChanged => {
            let original = raw.clone();
            let sanitized = sanitize_settings_value(raw)?;
            // Compare against the on-disk payload after normalizing both sides
            // through serialization: comparing the struct's in-memory values
            // directly misfires because f32 fields reserialize as imprecise
            // f64 numbers.
            let normalized_candidate: serde_json::Value =
                serde_json::from_slice(&serde_json::to_vec(&sanitized.settings)?)?;
...
mod tests {
    ...
    #[test]
    fn unchanged_settings_are_not_rewritten_on_load() {
        let tempdir = tempfile::tempdir().unwrap();
        let path = tempdir.path().join("settings.json");
        let mut store = SettingsStore::load_from_path(&path).unwrap();
        store.settings_mut().terminal.font_size = 18;
        ...
        // If the reload attempted its legacy rewrite-on-load, the injected
        // atomic-replace failure would surface as a load error instead.
    ...
    #[test]
    fn changed_settings_are_sanitized_and_persisted_on_load() {
        let tempdir = tempfile::tempdir().unwrap();
        let path = tempdir.path().join("settings.json");
        ...
        assert!(!load.recovered_from_corrupt_file);
        assert_eq!(load.settings.terminal.scrollback, 500);
        let raw: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(path).unwrap()).unwrap();
        assert_eq!(raw["settings"]["terminal"]["scrollback"], 500);
    }
