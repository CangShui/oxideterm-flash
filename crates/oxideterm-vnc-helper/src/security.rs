...
fn negotiate_rfb37_or_38_security(
    ...
    if should_prefer_legacy_vnc_auth(
        security_policy,
        password_available,
        security_types.as_slice(),
    ) {
        // Some TigerVNC configurations advertise TLSVnc without usable TLS
        // credentials. An explicit legacy policy permits the password-backed
        // VNC method, which is the reliable fallback for those servers.
        stream
...
fn should_prefer_legacy_vnc_auth(
    security_policy: RemoteDesktopVncSecurityPolicy,
    password_available: bool,
    security_types: &[u8],
) -> bool {
    security_policy == RemoteDesktopVncSecurityPolicy::AllowLegacy
        && password_available
        && security_types.contains(&VNC_SECURITY_VNC_AUTH)
}
...
mod tests {
    ...
    #[test]
    fn explicit_legacy_policy_prefers_vnc_auth_over_unusable_tlsvnc() {
        assert!(should_prefer_legacy_vnc_auth(
            RemoteDesktopVncSecurityPolicy::AllowLegacy,
            true,
            &[VNC_SECURITY_VNC_AUTH, VNC_SECURITY_VENCRYPT],
        ));
        assert!(!should_prefer_legacy_vnc_auth(
            RemoteDesktopVncSecurityPolicy::AllowUnverifiedEncryption,
            true,
