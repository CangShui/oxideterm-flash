...
fn main() {
    ...
    // The startup load is reused for workspace construction so the process
    // performs one settings read (and at most one rewrite) instead of two.
    let mut startup_settings_store = Some(match startup_settings_store {
        Ok(store) => store,
        Err(error) => {
            eprintln!("failed to load OxideTerm settings: {error:#}");
    ...
    application.on_open_urls(move |urls, cx| {
        let settings_store = load_or_fallback_settings_store();
        if !settings_store
            .settings()
            .general
            .external_connection_uris_enabled
    ...
    application.on_reopen(move |cx| {
        ...
        if let Err(error) = open_primary_window(
            ...
            load_or_fallback_settings_store(),
        ) {
            eprintln!(
                "oxideterm-flash could not reopen a native GPUI window: {error:#}\n\
    ...
    application.run(move |cx: &mut App| {
        ...
        let workspace_opened = match open_primary_window(
            ...
            startup_settings_store.take().unwrap_or_else(fallback_settings_store),
        ) {
            Ok(workspace_opened) => workspace_opened,
            Err(err) => {
                eprintln!(
                    "oxideterm-flash could not open a native GPUI window: {err:#}\n\
...
fn open_main_workspace_window(
    ...
    let window_ui = settings_store.settings().window_ui.clone();
...
fn fallback_settings_store() -> SettingsStore {
    // A failed startup load must still yield a usable workspace; the
    // read-only store serves defaults without rewriting the unreadable file.
    SettingsStore::from_read_only(
        oxideterm_settings::default_settings_path(),
        oxideterm_settings::PersistedSettings::default(),
    )
}

fn load_or_fallback_settings_store() -> SettingsStore {
    SettingsStore::load_default().unwrap_or_else(|error| {
        eprintln!("failed to load OxideTerm settings: {error:#}");
        fallback_settings_store()
    })
}
