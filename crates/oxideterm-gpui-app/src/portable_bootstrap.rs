...
use oxideterm_settings::{PersistedSettings, SettingsStore};
...
pub(crate) fn open_portable_bootstrap_window(
    ...
    if let Some(titlebar) = options.titlebar.as_mut() {
        titlebar.title = Some(SharedString::from("oxideterm-flash"));
...
impl PortableBootstrapWindow {
    ...
    fn finish_action(
        ...
        let Err(error) = result else {
            ...
            // Bootstrap may have relocated the data directory, so the main
            // workspace must load settings from the freshly selected root
            // instead of reusing the pre-bootstrap snapshot.
            let settings_store = match SettingsStore::load_default() {
                Ok(store) => store,
                Err(load_error) => {
                    self.error = Some(format!("failed to load settings: {load_error:#}"));
                    cx.notify();
                    return;
                }
            };
