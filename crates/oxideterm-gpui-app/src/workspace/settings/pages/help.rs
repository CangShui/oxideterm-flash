...
pub(in crate::workspace) const HELP_WEBSITE_URL: &str =
    "https://github.com/CangShui/oxideterm-flash";
...
pub(in crate::workspace) const HELP_ISSUES_URL: &str =
    "https://github.com/CangShui/oxideterm-flash/issues";
...
pub(in crate::workspace) const HELP_LEGAL_URL: &str =
    "https://github.com/CangShui/oxideterm-flash/blob/main/LEGAL.md";
...
impl WorkspaceApp {
    ...
    pub(in crate::workspace) fn open_help_url(
        ...
        if let Err(error) = open_external_url(url) {
            self.push_settings_toast(error.to_string(), TerminalNoticeVariant::Error, cx);
            cx.notify();
        }
    }
