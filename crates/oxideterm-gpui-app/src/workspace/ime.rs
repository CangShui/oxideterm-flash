...
impl WorkspaceApp {
    ...
    fn ime_target_horizontal_padding(target: WorkspaceImeTarget, control_padding_x: f32) -> Pixels {
        match target {
            WorkspaceImeTarget::Sftp(_) | WorkspaceImeTarget::ReadOnlyText(_) => {
...
mod tests {
    ...
    fn classified_secret_ime_targets_project_only_masked_utf16_geometry() {
        let secret = "token密😄";
        let targets = [
            WorkspaceImeTarget::Settings(SettingsInput::NetworkProxyPassword),
    ...
    fn platform_commit_and_marked_text_debug_are_redacted() {
        ...
        let mut pending = PendingPlatformTextCommit {
            target: WorkspaceImeTarget::Settings(SettingsInput::NetworkProxyPassword),
