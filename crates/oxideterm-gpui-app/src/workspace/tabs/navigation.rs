...
impl WorkspaceApp {
    ...
    pub(in crate::workspace) fn request_close_tab_by_id(
        ...
        if self.tabs(cx)[index].kind == TabKind::Telnet
            || self.tabs(cx)[index].kind == TabKind::Serial
        {
    ...
    fn request_local_terminal_close_check(
        ...
        let probes = {
            ...
            tab_ids
                .iter()
                .filter_map(|tab_id| {
                    self.tabs(cx).iter().find(|tab| {
                        tab.id == *tab_id
                            && (tab.kind == TabKind::Telnet || tab.kind == TabKind::Serial)
                    })
                })
