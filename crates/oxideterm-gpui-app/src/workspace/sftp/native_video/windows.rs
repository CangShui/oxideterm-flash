...
use ::windows::{
    Win32::{
        ...
        Media::MediaFoundation::{
            ...
            MFP_CREATION_OPTIONS, MFP_EVENT_HEADER, MFP_EVENT_TYPE_ERROR,
            ...
            MFP_MEDIAPLAYER_STATE, MFP_MEDIAPLAYER_STATE_EMPTY, MFP_MEDIAPLAYER_STATE_PAUSED,
    ...
    core::{BOOL, HRESULT, PCSTR, PCWSTR, w},
};
...
/// Creates the Media Foundation Playback player through `mfplay.dll`.
///
/// `mfplay.dll` was removed from Windows 11 24H2+, so the import is resolved
/// lazily instead of at process load; when the library is absent the SFTP
/// video preview reports an error while the rest of the app keeps working.
fn mfp_create_media_player(
    url: PCWSTR,
    start_playback: bool,
    creation_options: MFP_CREATION_OPTIONS,
    callback: *mut c_void,
    hwnd: HWND,
    player: *mut *mut c_void,
) -> windows_core::Result<()> {
    type MfpCreateMediaPlayer = unsafe extern "system" fn(
        PCWSTR,
        BOOL,
    ...
    let dll_name: Vec<u16> = "mfplay.dll\0".encode_utf16().collect();
    let module = unsafe { LoadLibraryW(PCWSTR(dll_name.as_ptr())) }?;
    let symbol_name: Vec<u8> = b"MFPCreateMediaPlayer\0".to_vec();
    let Some(proc) = (unsafe { GetProcAddress(module, PCSTR(symbol_name.as_ptr())) }) else {
        return Err(windows_core::Error::from_win32());
    };
    let function: MfpCreateMediaPlayer = unsafe { std::mem::transmute(proc) };
    unsafe {
        function(
            url,
            start_playback.into(),
...
impl SftpNativeVideoSurface {
    ...
    fn sync_inner(
        ...
        if self.path.as_deref() != Some(path) {
            ...
            .into();
            unsafe {
                mfp_create_media_player(
                    PCWSTR(url.as_ptr()),
                    true,
                    MFP_OPTION_NONE,
                    &callback as *const _ as *mut c_void,
                    hwnd,
                    &mut player as *mut Option<IMFPMediaPlayer> as *mut *mut c_void,
                )
