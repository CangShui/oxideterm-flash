...
impl ConnectionWorkspaceState {
    pub(super) fn initialize_oxide_delivery(&mut self, cx: &mut Context<Self>) {
        ...
        let delivery_task = cx.spawn(async move |connection_workspace, cx| {
            loop {
                task_wake.wait().await;
                let should_drain = task_wake.take();
                let stopped = task_wake.is_stopped();
                if should_drain {
                    let backlog_remaining = connection_workspace
                        .update(cx, |connection_workspace, cx| {
                            connection_workspace.drain_oxide_deliveries(cx)
                        })
                        .unwrap_or(false);
    ...
    fn drain_oxide_deliveries(&mut self, cx: &mut Context<Self>) -> bool {
        ...
        if !effects.is_empty() {
            cx.emit(ConnectionWorkspaceEvent::OxideEffectsReady(
    ...
    fn begin_import_dialog_exit(&mut self, delay: Duration, cx: &mut Context<Self>) -> bool {
        ...
        self.import_dialog_exit_task = Some(cx.spawn(async move |connection_workspace, cx| {
            Timer::after(delay).await;
            let _ = connection_workspace.update(cx, |connection_workspace, cx| {
                connection_workspace.finish_import_dialog_exit(generation, cx);
            });
        }));
        cx.notify();
        true
    ...
    fn begin_export_dialog_exit(&mut self, delay: Duration, cx: &mut Context<Self>) -> bool {
        ...
        self.export_dialog_exit_task = Some(cx.spawn(async move |connection_workspace, cx| {
            Timer::after(delay).await;
            let _ = connection_workspace.update(cx, |connection_workspace, cx| {
                connection_workspace.finish_export_dialog_exit(generation, cx);
            });
        }));
        cx.notify();
        true
    ...
    fn start_import_file_picker(
        ...
        self.import_file_picker_task = Some(cx.spawn(async move |connection_workspace, cx| {
            let result = selection.await;
            let _ = connection_workspace.update(cx, |connection_workspace, cx| {
                connection_workspace.import_file_picker_task = None;
                let Some(result) = result else {
                    return;
                };
                let Some(dialog) = connection_workspace.oxide_import_dialog.as_mut() else {
                    return;
                };
                match result {
    ...
    fn start_export_file_picker(
        ...
        self.export_file_picker_task = Some(cx.spawn(async move |connection_workspace, cx| {
            let result = save.await;
            let _ = connection_workspace.update(cx, |connection_workspace, cx| {
                connection_workspace.export_file_picker_task = None;
                let Some(result) = result else {
                ...
                match result {
                    Ok(path) => {
                        ...
                        connection_workspace.status = Some(summary);
                        connection_workspace.oxide_export_dialog = None;
    ...
    fn schedule_import_auto_close(&mut self, delay: Duration, cx: &mut Context<Self>) {
        ...
        self.dialog_auto_close_task = Some(cx.spawn(async move |connection_workspace, cx| {
            Timer::after(delay).await;
            let _ = connection_workspace.update(cx, |connection_workspace, cx| {
                let should_close =
                ...
                if should_close {
                    connection_workspace.oxide_import_dialog = None;
                    connection_workspace.focused_input = None;
                }
                connection_workspace.dialog_auto_close_task = None;
                cx.notify();
            });
        }));
    }
...
impl WorkspaceApp {
    pub(super) fn begin_oxide_import_dialog_exit(&mut self, cx: &mut Context<Self>) -> bool {
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.begin_import_dialog_exit(delay, cx)
        })
    }
    ...
    pub(super) fn begin_oxide_export_dialog_exit(&mut self, cx: &mut Context<Self>) -> bool {
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.begin_export_dialog_exit(delay, cx)
        })
    }
    ...
    pub(in crate::workspace) fn open_oxide_import_dialog(&mut self, cx: &mut Context<Self>) {
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            ...
            connection_workspace.focused_input = None;
            connection_workspace.status = None;
            cx.notify();
        });
    }
    ...
    pub(in crate::workspace) fn open_oxide_import_portable_migration_dialog(
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.oxide_import_dialog = Some(dialog);
            connection_workspace.focused_input = None;
            connection_workspace.status = None;
            cx.notify();
        });
    ...
    pub(in crate::workspace) fn active_session_manager_input(
        &self,
        cx: &App,
    ) -> Option<ConnectionWorkspaceInput> {
        let input = self.connection_workspace.read(cx).focused_input?;
        let session_manager_tab_active = false;
        let connection_workspace = self.connection_workspace.read(cx);
        session_manager_input_is_active(
            input,
            session_manager_tab_active,
            connection_workspace.oxide_import_dialog.as_ref(),
            connection_workspace.oxide_export_dialog.as_ref(),
        )
        .then_some(input)
    }
    ...
    pub(in crate::workspace) fn focused_oxide_dialog_input(
        ...
        let connection_workspace = self.connection_workspace.read(cx);
        let input = connection_workspace.focused_input?;
        session_manager_input_is_active(
            input,
            false,
            connection_workspace.oxide_import_dialog.as_ref(),
    ...
    pub(super) fn open_oxide_export_dialog_with_portable_mode(
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.oxide_export_dialog = Some(dialog);
            connection_workspace.focused_input = None;
            connection_workspace.status = None;
            cx.notify();
        });
    ...
    pub(in crate::workspace) fn handle_oxide_dialog_footer_key(
        ...
        let (has_import_dialog, import_exiting, has_export_dialog, export_exiting) = {
            let connection_workspace = self.connection_workspace.read(cx);
            (
                connection_workspace.oxide_import_dialog.is_some(),
                connection_workspace
                    .oxide_import_dialog
                    .as_ref()
                    .is_some_and(|dialog| {
                ...
                connection_workspace.oxide_export_dialog.is_some(),
                connection_workspace
                    .oxide_export_dialog
                    .as_ref()
                    .is_some_and(|dialog| {
    ...
    pub(super) fn handle_oxide_import_footer_key(
            ...
            connection_workspace
                ...
                .and_then(|dialog| {
                    ...
                    (!actions.is_empty()).then(|| {
                        ...
                        let current_input = connection_workspace
                            .focused_input
        ...
        match browser_behavior::modal_footer_body_input_key_action(
            ...
            Some(browser_behavior::ModalFooterBodyInputKeyAction::FocusInput(input)) => {
                self.connection_workspace.update(cx, |connection_workspace, cx| {
                    connection_workspace.focused_input = Some(input);
                    if let Some(dialog) = connection_workspace.oxide_import_dialog.as_mut() {
    ...
    pub(super) fn activate_oxide_import_footer_action(
        ...
        let Some((preview_ready, result_ready, has_selected_content, has_password)) = ({
            let connection_workspace = self.connection_workspace.read(cx);
            connection_workspace.oxide_import_dialog.as_ref().map(|dialog| {
                (
    ...
    pub(super) fn select_oxide_import_file(&mut self, cx: &mut Context<Self>) {
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.start_import_file_picker(selection, cx);
        });
    }
    ...
    pub(super) fn preview_oxide_import_dialog(&mut self, cx: &mut Context<Self>) {
        ...
        let Some((bytes, password, conflict_strategy, generation, sender)) =
            self.connection_workspace.update(cx, |connection_workspace, cx| {
                let Some(sender) = connection_workspace.oxide_worker_sender() else {
                    return None;
                };
                let Some(dialog) = connection_workspace.oxide_import_dialog.as_mut() else {
                    return None;
        ...
        self.connection_workspace.update(cx, |connection_workspace, _cx| {
            connection_workspace.retain_oxide_worker(OxideWorkerKey::Preview(generation), worker);
        });
    }
    ...
    pub(super) fn apply_oxide_import_dialog(&mut self, cx: &mut Context<Self>) {
        ...
        self.connection_workspace.update(cx, |connection_workspace, _cx| {
            connection_workspace.retain_oxide_worker(OxideWorkerKey::Import(generation), worker);
        });
    }

    pub(in crate::workspace) fn handle_session_manager_workspace_event(
        &mut self,
        event: &ConnectionWorkspaceEvent,
        cx: &mut Context<Self>,
    ) {
        match event {
            ConnectionWorkspaceEvent::OxideEffectsReady(effects) => {
                for effect in effects.take() {
                    self.handle_oxide_workspace_effect(effect, cx);
                }
            }
            ConnectionWorkspaceEvent::RefreshOxideExportPreflight => {
    ...
    fn handle_oxide_workspace_effect(
        ...
        match effect {
            ...
            OxideWorkspaceEffect::ExportDone {
                ...
                Ok(bytes) => {
                    drop(password);
                    let exported_count = self.connection_workspace.update(cx, |connection_workspace, cx| {
    ...
    pub(super) fn present_oxide_import_result(
        ...
        let result_view = OxideImportResultView {
            ...
            // Plugin settings and portable AI provider keys are no longer
            // imported; these stay zero so the result view remains complete.
            imported_plugin_settings: 0,
            skipped_plugin_settings: false,
            imported_portable_secrets: 0,
            skipped_portable_secrets: 0,
        };
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            ...
            connection_workspace.status = Some(result_summary);
            if auto_close_import_dialog {
                connection_workspace.schedule_import_auto_close(Duration::from_secs(2), cx);
            }
            cx.notify();
        });
    }
    ...
    pub(super) fn oxide_export_preflight_for_dialog(
        ...
        let has_preflight_content = !self.oxide_export_connection_ids(dialog).is_empty();
    ...
    pub(super) fn export_oxide_dialog(&mut self, cx: &mut Context<Self>) {
        ...
        let Some((password, generation, sender)) =
            self.connection_workspace.update(cx, |connection_workspace, cx| {
                ...
                let dialog = connection_workspace.oxide_export_dialog.as_mut()?;
        ...
        self.connection_workspace.update(cx, |connection_workspace, _cx| {
            connection_workspace.retain_oxide_worker(OxideWorkerKey::Export(generation), worker);
        });
    }
    ...
    pub(super) fn prompt_save_oxide_export(
        ...
        self.connection_workspace.update(cx, |connection_workspace, cx| {
            connection_workspace.start_export_file_picker(
                save,
                settings_path,
                success_template,
                exported_count,
...
mod tests {
    ...
    fn oxide_delivery_updates_progress_without_a_workspace(cx: &mut TestAppContext) {
        ...
        let sender = connection_workspace.read_with(cx, |connection_workspace, _cx| {
            connection_workspace
                .oxide_worker_sender()
                .expect("oxide worker sender")
        });
        connection_workspace.update(cx, |connection_workspace, _cx| {
        ...
        connection_workspace.read_with(cx, |connection_workspace, _cx| {
            let progress = connection_workspace
                .oxide_import_dialog
                .as_ref()
