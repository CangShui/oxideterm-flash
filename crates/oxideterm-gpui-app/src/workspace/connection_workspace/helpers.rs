    ...
    let (auth_tab, password, key_path, managed_key_id, cert_path, passphrase, save_password) =
        match conn.auth.conventional_fallback() {
            SavedAuth::Password {
                ...
                String::new(),
                String::new(),
                true, // Default to true so newly entered passwords are saved automatically
            ),
            SavedAuth::Key {
                key_path,
                has_passphrase,
...
fn connection_draft_from_form_with_proxy_hop_prefix(
    ...
    ConnectionDraft {
        ...
        icon: if form.icon.trim().is_empty() {
            default_connection_transport_icon_id(
                oxideterm_connections::ConnectionTransport::Ssh,
            )
            .to_string()
        } else {
            form.icon.clone()
