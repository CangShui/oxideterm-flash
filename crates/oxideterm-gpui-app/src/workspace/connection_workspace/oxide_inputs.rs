...
impl WorkspaceApp {
    pub(in crate::workspace) fn render_session_text_input(
        &self,
        target: ConnectionWorkspaceInput,
        value: &str,
        placeholder: String,
        cx: &mut Context<Self>,
    ) -> AnyElement {
    ...
    pub(super) fn render_session_text_input_with_options(
        ...
        let active = self.connection_workspace.read(cx).focused_input() == Some(target);
        let has_background = self.background_surface_active("connection_workspace");
        let marked = self
    ...
    pub(in crate::workspace) fn render_session_password_input(
        ...
        let masked_value = {
            ...
            let value = match target {
                ConnectionWorkspaceInput::OxideExportPassword => manager
                    .oxide_export_dialog
                    .as_ref()
                    .map(|dialog| dialog.password.as_str()),
                ConnectionWorkspaceInput::OxideExportConfirmPassword => manager
                    .oxide_export_dialog
                    .as_ref()
                    .map(|dialog| dialog.confirm_password.as_str()),
                ConnectionWorkspaceInput::OxideImportPassword => manager
                    .oxide_import_dialog
                    .as_ref()
                    .map(|dialog| dialog.password.as_str()),
                _ => None,
            ...
            // secret remains owned by the connection workspace state.
            text_input_secret_mask(value)
        };
