...
impl WorkspaceApp {
    ...
    // Trigger evaluation needs the owning session of a pane, but the trigger
    // runtime keeps no pane-to-session registry of its own; walk each tab's
    // pane tree instead so resolution stays owned by the tab host data.
    fn terminal_trigger_session_id_for_pane(
        &self,
        pane_id: PaneId,
        cx: &App,
    ) -> Option<TerminalSessionId> {
        self.tabs(cx).iter().find_map(|tab| {
            tab.root_pane
                .as_ref()
    ...
    pub(in crate::workspace) fn terminal_trigger_effective_enabled_for_pane(
        ...
        let Some(session_id) = self.terminal_trigger_session_id_for_pane(pane_id, cx) else {
            return false;
        };
    ...
    pub(in crate::workspace) fn terminal_trigger_applies_to_pane(
        ...
        let saved_connection = self
            .terminal_trigger_session_id_for_pane(pane_id, cx)
