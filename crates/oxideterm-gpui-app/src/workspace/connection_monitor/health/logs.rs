...
impl WorkspaceApp {
    pub(super) fn render_host_logs_panel(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Context columns required a sidebar wider than the removed AI
        // workspace could ever grow, so the host tools panel keeps them hidden.
        let show_context_columns = false;
        let search_ime = self
