...
impl WorkspaceRuntimeEntity {
    ...
    fn reduce_node_event(&mut self, event: NodeStateEvent) -> Option<NodeRuntimeEffect> {
        ...
        match event {
            ...
            // The shared SFTP session lifecycle no longer has a runtime
            // consumer; the event is only drained to keep generation tracking.
            NodeStateEvent::SharedSftpSessionChanged { .. } => None,
