...
use oxideterm_settings::{
    PersistedSettings, RECOMMENDED_FOCUS_HANDOFF_COMMANDS,
    SettingsUpstreamProxyAuth, UpdateProxyMode, reindex_highlight_rules,
};
use oxideterm_terminal_semantic::SEMANTIC_CLASSES;

use crate::{SettingsInput, edit_custom_semantic_scheme};

// Formerly lived in the removed plugin module; this parser only normalizes
// the terminal command-bar focus handoff allowlist.
...
pub fn apply_persisted_settings_input_draft(
    ...
    match input {
        ...
        SettingsInput::ExternalEditorPath => {
            settings.general.external_editor = draft.trim().to_string();
            SettingsInputDraftApply::Applied
        }
