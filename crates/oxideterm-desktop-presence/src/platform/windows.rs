...
fn main_window_hwnd(window: &Window) -> anyhow::Result<HWND> {
    ...
    let RawWindowHandle::Win32(handle) = handle.as_raw() else {
        return Err(anyhow!("oxideterm-flash main window is not a Win32 window"));
    };
    ...
    thread::Builder::new()
        ...
        .spawn(|| {
            if let Err(error) = run_tray_message_loop() {
                eprintln!("failed to start oxideterm-flash Windows tray icon: {error:#}");
            }
        })
...
fn run_tray_message_loop() -> anyhow::Result<()> {
    ...
    let class_name = w!("OxidetermFlashTrayWindow");
...
unsafe extern "system" fn tray_window_proc(
    ...
    if is_taskbar_created_message(message, TASKBAR_CREATED_MESSAGE.load(Ordering::SeqCst)) {
        ...
        if let Err(error) = add_tray_icon(hwnd) {
            eprintln!("failed to restore oxideterm-flash tray icon: {error:#}");
        }
        return LRESULT(0);
    }
