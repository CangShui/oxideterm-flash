...
impl ThemeTokens {
    ...
    /// Recompute image-background tint strength after a palette change. Light
    /// backgrounds need more tint than dark ones to keep text and surfaces
    /// separated from arbitrary image luminance.
...
mod tests {
    ...
    fn applies_builtin_theme_overrides() {
        let ui = ThemeTokens::from_builtin(theme_by_id("default")).ui;
        assert_eq!(ui.bg_panel, 0x18181b);
        assert_eq!(ui.bg_card, 0x1e1e22);
        assert_eq!(ui.border, 0x2e2e33);
        assert_eq!(ui.text_muted, 0xa1a1aa);
    }
