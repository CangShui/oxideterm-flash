...
fn certificate_algorithm_name(cert: &Certificate, hash_alg: Option<HashAlg>) -> String {
    match (cert.algorithm(), hash_alg) {
        (Algorithm::Rsa { .. }, Some(HashAlg::Sha256)) => {
            "rsa-sha2-256-cert-v01@openssh.com".to_owned()
        }
        (Algorithm::Rsa { .. }, Some(HashAlg::Sha512)) => {
            "rsa-sha2-512-cert-v01@openssh.com".to_owned()
        }
        _ => cert.algorithm().to_certificate_type(),
    }
}
                    ...
                    match buf.split_first() {
                        ...
                        Some((&msg::USERAUTH_INFO_REQUEST_OR_USERAUTH_PK_OK, mut r)) => {
                            if let Some(auth::CurrentRequest::GssapiWithMic) = auth_request.current
                            {
                                ...
                                map_err!(ensure_end(&r))?;
                                let mic_data =
                                    enc.client_make_gssapi_mic_data(&self.common.auth_user)?;
                                self.sender
                        ...
                        Some((&msg::USERAUTH_GSSAPI_TOKEN, mut r)) => {
                            if let Some(auth::CurrentRequest::GssapiWithMic) = auth_request.current
                            {
                                ...
                                let token = Zeroizing::new(
                                    map_err!(Bytes::decode(&mut r))?.to_vec(),
                                );
                                map_err!(ensure_end(&r))?;
                                let mic_data =
                                    enc.client_make_gssapi_mic_data(&self.common.auth_user)?;
                        ...
                        Some((&msg::USERAUTH_GSSAPI_ERROR, mut r)) => {
                            if let Some(auth::CurrentRequest::GssapiWithMic) = auth_request.current
                            {
                                ...
                                let message =
                                    Zeroizing::new(map_err!(String::decode(&mut r))?);
                                let _language_tag =
                                    Zeroizing::new(map_err!(String::decode(&mut r))?);
                                map_err!(ensure_end(&r))?;
                                warn!(
...
mod tests {
    ...
    fn client_make_gssapi_mic_data_matches_rfc_4462() {
        let mut encrypted = test_encrypted();
        let mic_data = encrypted.client_make_gssapi_mic_data("alice").unwrap();
        ...
        assert_eq!(mic_data.as_slice(), expected.as_slice());
    }
...
impl Encrypted {
        ...
        Ok(push_packet!(self.write, {
            ...
            match *auth_method {
                ...
                auth::Method::OpenSshCertificate { ref cert, .. } => {
                    ...
                    certificate_algorithm_name(cert, None).encode(&mut self.write)?;
                ...
                auth::Method::FutureCertificate { ref cert, hash_alg } => {
                    ...
                    certificate_algorithm_name(cert, hash_alg).encode(&mut self.write)?;
                    cert.to_bytes()?.as_slice().encode(&mut self.write)?;
                    true
                }
    ...
    fn client_make_gssapi_mic_data(
        ...
        // The MIC input contains serialized authentication context and crosses
        // an async queue, so it owns one zeroizing buffer without a plain copy.
        let mut mic_data = Zeroizing::new(Vec::new());
        self.session_id.as_ref().encode(&mut *mic_data)?;
        mic_data.push(msg::USERAUTH_REQUEST);
        user.encode(&mut *mic_data)?;
        "ssh-connection".encode(&mut *mic_data)?;
        "gssapi-with-mic".encode(&mut *mic_data)?;
        Ok(mic_data)
    }
    ...
    fn client_make_to_sign(
        ...
        match key {
            PublicKeyOrCertificate::Certificate(cert) => {
                certificate_algorithm_name(cert, cert_hash_alg).encode(buffer)?;
