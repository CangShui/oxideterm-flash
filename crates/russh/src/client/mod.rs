...
impl fmt::Debug for Reply {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::AuthSuccess => formatter.write_str("AuthSuccess"),
            Self::AuthFailure {
                proceed_with_methods,
                partial_success,
            } => formatter
                .debug_struct("AuthFailure")
                .field("proceed_with_methods", proceed_with_methods)
                .field("partial_success", partial_success)
                .finish(),
            Self::ChannelOpenFailure => formatter.write_str("ChannelOpenFailure"),
            Self::SignRequest { key, .. } => formatter
                .debug_struct("SignRequest")
                .field("key", key)
                .field("data", &"<redacted auth payload>")
                .finish(),
            Self::SignRequestCert {
                cert, hash_alg, ..
            } => formatter
                .debug_struct("SignRequestCert")
                .field("cert", cert)
                .field("hash_alg", hash_alg)
                .field("data", &"<redacted auth payload>")
                .finish(),
            Self::AuthInfoRequest { prompts, .. } => formatter
                .debug_struct("AuthInfoRequest")
                .field("prompt_count", &prompts.len())
                .finish(),
            Self::AuthGssapiResponse {
                selected_mechanism, ..
            } => formatter
                .debug_struct("AuthGssapiResponse")
                .field("selected_mechanism", selected_mechanism)
                .field("mic_data", &"<redacted auth payload>")
                .finish(),
            Self::AuthGssapiToken { .. } => formatter
                .debug_struct("AuthGssapiToken")
                .field("token", &"<redacted>")
                .field("mic_data", &"<redacted auth payload>")
                .finish(),
            Self::AuthGssapiError { error } => formatter
                .debug_struct("AuthGssapiError")
                .field("error", error)
                .finish(),
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            Self::AuthInfoResponse { .. } => f
                ...
                .finish(),
            Self::AuthGssapiToken { .. } => f
                .debug_struct("AuthGssapiToken")
                .field("token", &"<redacted>")
                .finish(),
            Self::AuthGssapiMic { token, .. } => f
                .debug_struct("AuthGssapiMic")
                .field("token", &token.as_ref().map(|_| "<redacted>"))
                .field("mic", &"<redacted>")
                .finish(),
            Self::AuthGssapiExchangeComplete { token } => f
                .debug_struct("AuthGssapiExchangeComplete")
                .field("token", &token.as_ref().map(|_| "<redacted>"))
                .finish(),
            ...
            Self::ServerChannelOpenReply { result, .. } => f
                .debug_struct("ServerChannelOpenReply")
                .field("result", result)
                .finish(),
...
impl Session {
    ...
    fn handle_msg(&mut self, msg: Msg) -> Result<(), crate::Error> {
        match msg {
            ...
            Msg::AuthGssapiMic { token, mic } => {
                if let Some(ref mut enc) = self.common.encrypted {
                    enc.client_send_gssapi_mic(
                        token.as_ref().map(|token| token.as_slice()),
                        &mic,
                    )?;
                }
            }
            Msg::AuthGssapiExchangeComplete { token } => {
                if let Some(ref mut enc) = self.common.encrypted {
                    enc.client_send_gssapi_exchange_complete(
                        token.as_ref().map(|token| token.as_slice()),
                    )?;
                }
            }
