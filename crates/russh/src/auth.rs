...
pub enum GssapiStep {
    ...
    Complete {
        ...
        mic: Option<Zeroizing<Vec<u8>>>,
    },
}

impl fmt::Debug for GssapiStep {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Continue { .. } => formatter
                .debug_struct("Continue")
                .field("token", &"<redacted>")
                .finish(),
            Self::Complete { token, mic } => formatter
                .debug_struct("Complete")
                .field("token", &token.as_ref().map(|_| "<redacted>"))
                .field("mic", &mic.as_ref().map(|_| "<redacted>"))
                .finish(),
        }
    }
}
...
pub enum GssapiError {
    ...
    /// error details.
    ErrorToken(Zeroizing<Vec<u8>>),
}

impl fmt::Debug for GssapiError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Status {
                major_status,
                ...
                .field("minor_status", minor_status)
                .field("message", &"<redacted>")
                .finish(),
            Self::ErrorToken(_) => formatter
                .debug_tuple("ErrorToken")
                .field(&"<redacted>")
                .finish(),
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::KeyboardInteractive { submethods } => f
                ...
                .finish(),
            Self::GssapiWithMic { mechanism_oids } => f
                .debug_struct("GssapiWithMic")
                .field("mechanism_count", &mechanism_oids.len())
                .finish(),
        }
    }
}
