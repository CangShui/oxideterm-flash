...
impl GraphicsPipelineClient {
    ...
    fn decode_avc420(&mut self, surface_id: u16, dest_rect: &ExclusiveRectangle, bitmap_data: &[u8]) -> PduResult<()> {
        ...
        // MS-RDPEGFX 2.2.4.4: only the regionRects areas of the H.264 frame
        // contain valid, updated image data; everything else inside the
        // destination bounding box holds stale macroblock padding and must not
        // be written. Apply each region separately; when the server sends no
        // regions, conservatively keep the legacy full-destination blit.
        if !stream.rectangles.is_empty() {
            for region in &stream.rectangles {
                // Clip the region to the destination rectangle: malformed
                // servers may send regions that spill outside the frame.
                let left = region.left.max(dest_rect.left);
                let top = region.top.max(dest_rect.top);
                let right = region.right.min(dest_rect.right);
                let bottom = region.bottom.min(dest_rect.bottom);
                if right <= left || bottom <= top {
                    continue;
                }
                let offset_x = left.saturating_sub(dest_rect.left);
                let offset_y = top.saturating_sub(dest_rect.top);
                let region_width = right - left;
                let region_height = bottom - top;
                let region_data = extract_rgba_region(
                    &cropped_data,
                    dest_width,
                    offset_x,
                    offset_y,
                ...
                let region_rect = ExclusiveRectangle {
                    left,
                    top,
                    right,
                    bottom,
                };
                self.compositor.apply_bitmap(surface_id, &region_rect, &region_data);
                let update = BitmapUpdate {
                    surface_id,
                    destination_rectangle: region_rect,
...
/// Extracts a sub-region from tightly packed RGBA data. Row stride is
/// `source_width * 4` bytes; out-of-range requests yield an empty buffer so a
/// malformed regionRect cannot panic the session.
fn extract_rgba_region(
    ...
    if source_width == 0 || width == 0 || height == 0 {
        return Vec::new();
    }
    let source_stride = usize::from(source_width) * 4;
    let region_stride = usize::from(width) * 4;
    let mut region = Vec::with_capacity(region_stride.saturating_mul(usize::from(height)));
    for row in 0..height {
        let row_index = usize::from(offset_y.saturating_add(row));
        let start = row_index
            .saturating_mul(source_stride)
            .saturating_add(usize::from(offset_x) * 4);
        let end = start.saturating_add(region_stride);
        if end <= data.len() {
            region.extend_from_slice(&data[start..end]);
        } else if start < data.len() {
            region.extend_from_slice(&data[start..]);
            region.resize(region_stride.saturating_mul(usize::from(row) + 1), 0);
        } else {
            region.resize(region_stride.saturating_mul(usize::from(row) + 1), 0);
        }
    }
...
mod tests {
    ...
    #[test]
    fn extract_rgba_region_returns_the_requested_subregion() {
        // 4x2 RGBA source with a distinct color per pixel.
        let mut source = Vec::new();
        for pixel in 0..8u8 {
            source.extend_from_slice(&[pixel, pixel, pixel, 0xFF]);
        }
        let region = extract_rgba_region(&source, 4, 1, 0, 2, 1);
        // Pixels at (1,0) and (2,0) hold values 1 and 2.
        assert_eq!(region, vec![1, 1, 1, 0xFF, 2, 2, 2, 0xFF]);
    }

    #[test]
    fn extract_rgba_region_zero_fills_out_of_range_requests() {
        let source = vec![0x11u8; 4];
        let region = extract_rgba_region(&source, 1, 0, 8, 1, 1);
        // The requested row lies beyond the source; the result is zero-filled
        // with the requested size and never panics.
