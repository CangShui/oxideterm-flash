...
impl Processor {
    ...
    pub fn process_bitmap_update(
        ...
        let mut buf = Vec::new();
        let mut stretched_buf: Vec<u8> = Vec::new();
        let mut processor_updates = Vec::new();
        ...
        for update in bitmap_update.rectangles {
            ...
            // MS-RDPBCGR allows the source bitmap dimensions to differ from the
            // destination rectangle; the client must stretch the decoded data.
            // xrdp relies on this for glyph-heavy regions.
            let stretched = update.width != update_rectangle.width()
                || update.height != update_rectangle.height();
            buf.clear();
            stretched_buf.clear();
                ...
                .contains(ironrdp_pdu::bitmap::Compression::BITMAP_COMPRESSION)
            {
                if update.bits_per_pixel == 32 {
                    ...
                    match self.bitmap_stream_decoder.decode_bitmap_stream_to_rgb24(
                        ...
                        Ok(()) => {
                            // The stride must describe the buffer actually being
                            // applied: the stretched buffer is destination-sized,
                            // so passing the stale source width would fail the
                            // length validation and misalign every row.
                            let stride = if stretched {
                                update_rectangle.width()
                            } else {
                                update.width
                            };
                            let decoded = if stretched {
                                stretch_rows_bpp(
                                    &buf,
                                    usize::from(update.width),
                                    usize::from(update.height),
                                    usize::from(update_rectangle.width()),
                                    usize::from(update_rectangle.height()),
                                    3,
                                    &mut stretched_buf,
                                );
                                stretched_buf.as_slice()
                            } else {
                                &buf
                            };
                            apply_bitmap(image.apply_rgb24(decoded, &update_rectangle, stride, true))?
                        }
                        Err(err) => {
                    ...
                    match ironrdp_graphics::rle::decompress(
                        ...
                        Ok(format) => {
                            let bytes_per_pixel = match format {
                                RlePixelFormat::Rgb8 => 1usize,
                                RlePixelFormat::Rgb15 | RlePixelFormat::Rgb16 => 2usize,
                                RlePixelFormat::Rgb24 => 3usize,
                            };
                            let decoded = if stretched {
                                stretch_rows_bpp(
                                    &buf,
                            ...
                            match format {
                                RlePixelFormat::Rgb8 => apply_bitmap(image.apply_rgb8_with_palette(
                                    decoded,
                                    &update_rectangle,
                                    self.palette.colors(),
                                    stride,
                                ...
                                RlePixelFormat::Rgb15 => {
                                    apply_bitmap(image.apply_rgb15_bitmap(decoded, &update_rectangle, stride))?
                                }
                                RlePixelFormat::Rgb16 => {
                                    apply_bitmap(image.apply_rgb16_bitmap(decoded, &update_rectangle, stride))?
                                }
                                RlePixelFormat::Rgb24 => {
                                    apply_bitmap(image.apply_bgr24_bitmap(decoded, &update_rectangle, stride))?
                                }
                            }
                        }
                        Err(e) => {
                ...
                let src_width = usize::from(update.width);
                let src_height = usize::from(update.height);
                let row_bytes = src_width * bytes_per_pixel;
                let padded_row_bytes = (row_bytes + 3) & !3;

                // Normalize both padded and unpadded payloads into tightly
                // packed source rows so the stretch below sees one layout.
                let packed;
                let packed_data: &[u8] = if padded_row_bytes != row_bytes {
                    buf.reserve(src_height * row_bytes);
                    for row in update
                        .bitmap_data
                    ...
                    packed = std::mem::take(&mut buf);
                    &packed
                } else {
                    update.bitmap_data
                };

                let (decoded, stride) = if stretched {
                    stretch_rows_bpp(
                        packed_data,
                        src_width,
                        src_height,
                    ...
                    (stretched_buf.as_slice(), update_rectangle.width())
                } else {
                    (packed_data, update.width)
                };
                ...
                match update.bits_per_pixel {
                    ...
                    15 => apply_bitmap(image.apply_rgb15_bitmap(decoded, &update_rectangle, stride))?,
                    16 => apply_bitmap(image.apply_rgb16_bitmap(decoded, &update_rectangle, stride))?,
                    24 => apply_bitmap(image.apply_bgr24_bitmap(decoded, &update_rectangle, stride))?,
            ...
            if let Some(applied_rectangle) = applied_rectangle {
                processor_updates.push(UpdateKind::Region(applied_rectangle));
            }
        }
...
mod tests {
    ...
    #[test]
    fn stretched_rgb24_applies_with_destination_stride() {
        // A 2x1 source stretched to 4x1 must apply through the destination
        // stride: passing the stale source width fails the length validation
        // and misaligns rows (the regression fixed in process_bitmap_update).
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 4, 1);
        let update_rectangle = InclusiveRectangle {
            left: 0,
            top: 0,
            right: 3,
            bottom: 0,
        };
        // Two source pixels (red, green) stretched across four destinations.
        let source = vec![0xFF, 0x00, 0x00, 0x00, 0xFF, 0x00];
        let mut stretched = Vec::new();
        stretch_rows_bpp(&source, 2, 1, 4, 1, 3, &mut stretched);
        assert_eq!(stretched.len(), 4 * 3);
        let applied =
            image.apply_rgb24(&stretched, &update_rectangle, update_rectangle.width(), true);
        assert!(applied.is_ok());
        // RgbA32 stores R first. Nearest neighbor maps the four destination
        // columns onto two source columns as 0,0,1,1: red, red, green, green.
        let data = image.data();
        assert_eq!(&data[0..4], &[0xFF, 0x00, 0x00, 0xFF]);
        assert_eq!(&data[4..8], &[0xFF, 0x00, 0x00, 0xFF]);
        assert_eq!(&data[8..12], &[0x00, 0xFF, 0x00, 0xFF]);
        assert_eq!(&data[12..16], &[0x00, 0xFF, 0x00, 0xFF]);
    }
    ...
    fn malformed_bitmap_content_requests_recovery_without_terminating_the_session() {
        ...
        // The destination is valid under the stretch contract, so an empty
        // payload is an invalid source length: the update is skipped without
        // a recovery request (recovery is reserved for invalid destinations).
        assert!(!processor.take_bitmap_recovery_request());
    }
...
/// Nearest-neighbor stretch of tightly packed rows from the decoded source
/// size to the declared destination size. MS-RDPBCGR requires the client to
/// fit bitmap data to the destination rectangle; servers such as xrdp rely
/// on this for glyph-heavy regions.
fn stretch_rows_bpp(
    src: &[u8],
    src_width: usize,
    src_height: usize,
    ...
    dst.clear();
    dst.reserve(dst_width * dst_height * bytes_per_pixel);
    if src_width == 0 || src_height == 0 || dst_width == 0 || dst_height == 0 {
        return;
    }
    for dst_y in 0..dst_height {
        let src_y = dst_y * src_height / dst_height;
        let row_start = src_y * src_width * bytes_per_pixel;
        for dst_x in 0..dst_width {
            let src_x = dst_x * src_width / dst_width;
            let start = row_start + src_x * bytes_per_pixel;
            dst.extend_from_slice(&src[start..start + bytes_per_pixel]);
        }
    }
}
