...
impl DecodedImage {
    ...
    pub(crate) fn bitmap_destination(
        ...
        // destination rectangle; the client must stretch the data to fit.
        // Servers such as xrdp rely on this for glyph-heavy regions, so a size
        // mismatch must not reject the update outright.
        let _ = (width, height);
...
mod tests {
    ...
    fn bitmap_application_rejects_invalid_rectangles_and_source_lengths() {
        ...
        // MS-RDPBCGR requires the client to stretch mismatched source data, so
        // a 1x1 source against this 2x2 destination is accepted (the caller
        // stretches before applying); undersized raw data still errors.
        assert_eq!(
            image.bitmap_destination(&encoded_rectangle, 1, 1),
            Some(encoded_rectangle.clone())
        );
        assert!(
            image.apply_rgb16_bitmap(&[0; 6], &encoded_rectangle, 1).is_err(),
            "undersized bitmap data must not be applied"
        );
    }
}
