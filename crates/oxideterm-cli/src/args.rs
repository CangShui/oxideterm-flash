...
#[command(about = "oxideterm-flash headless management CLI")]
#[command(
...
pub enum Command {
    ...
    Ssh(SshLaunchArgs),
    #[command(
        about = "Open an ssh://, telnet://, rdp://, or vnc:// URI in the native GUI"
    )]
    ...
    Diagnose(OutputArgs),
    #[command(about = "Run health checks for settings and connections")]
    Doctor(DoctorArgs),
