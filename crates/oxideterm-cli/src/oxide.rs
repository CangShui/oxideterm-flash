...
use oxideterm_connections::{
    ...
    oxide_file::{
        ImportConflictStrategy, ImportPreview, ImportResultEnvelope, OxideExportOptions,
        OxideFile, OxideForwardRecord, OxideImportOptions, OxideMetadata,
...
fn export(args: OxideExportArgs) -> CliResult<i32> {
    ...
    let preflight = preflight_export(&store, &connection_ids, args.embed_keys, true, 0);
...
fn format_validate_text(response: &OxideValidateResponse) -> String {
    format!(
        "valid: true connections={} appSettings={}",
...
fn format_preview_text(response: &OxidePreviewImportResponse) -> String {
    format!(
        "connections={} strategy={} rename={} skip={} replace={} merge={} appSettings={} forwards={}",
...
fn format_import_text(response: &OxideImportResponse) -> String {
    ...
    format!(
        "applied: {} dryRun={} backup={} imported={} renamed={} replaced={} merged={} skipped={} appSettings={} quickCommands={}",
        response.applied,
        response.dry_run,
        response.backup_path.as_deref().unwrap_or("-"),
