...
use crate::{
    ...
    paths::{default_connections_path, default_settings_path},
    settings,
...
pub(super) fn restore(args: BackupRestoreArgs) -> CliResult<i32> {
    ...
    let has_changes = restore_inputs.iter().any(|input| !input.changes.is_empty());
    let mut guard = write_guard::prepare_write(&write, has_changes)?;

    if has_changes && !write.dry_run {
...
fn selected_sections(section: Option<BackupInspectSection>) -> Vec<BackupInspectSection> {
    section
        ...
        .unwrap_or_else(|| vec![BackupInspectSection::Settings, BackupInspectSection::Connections])
}
